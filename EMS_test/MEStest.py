    ...
    def provide_power(self, user_provided_time):
        ...
        if self.tou_price == "尖峰":
            ...
            if self.pile_load_demand > 0:
                ...
                self.grid_provide_power_for_pile = self.grid.provide_power(
                    self.pile_load_demand, '充電樁')
        ...
        self.grid_provide_power = self.grid_provide_power_for_pile + \
            self.grid_provide_power_for_ess
        return (self.grid_provide_power, -self.ess_provide_power)
    ...
    def __init__(self):
        ...
        for group in range(1, num_groups + 1):
            for sub_group in range(1, 3):  # 這裡使用 1 和 2 作為子組的標識
                self.pile_number.append(f"{group}-{sub_group}")
...
for time_steps in range(24):
    user_provided_time = datetime(
        year=2023, month=12, day=15, hour=time_steps, minute=30)
    tou_instance = TOU(current_time=user_provided_time)
