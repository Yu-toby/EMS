...
# If SoC is lower than 40%, it will charge more 2 to 4 hours
...
def generate_charging_data(days, original_data, min_records_per_day, max_records_per_day, soc_end_inf, soc_end_sup):
    ...
    for date in generated_dates:
        ...
        while records_generated < num_records_today:
            ...
            soc_end = np.random.randint(soc_end_inf, soc_end_sup + 1)
            ...
            if hour in soc_distribution_models and np.random.rand() < time_slot_distribution_model.get(hour, 0):
                ...
                soc_value = round(max(0, min(soc_end-1, soc_value + soc_random_offset)))  # Ensure SoC is within 0-soc_end-1% and round to the nearest integer

                rounded_time = round_time_to_nearest_quarter_hour(time(hour, quarter + minute_offset))

                if rounded_time <= time(15, 0) and rounded_time >= time(5, 0):
                    if number_of_morning < number_of_bus:
                        ...
                        if soc_value > 40:
                            ...
                            num_intervals = random.randint(8, 16)
                            interval_minutes = 15
                            total_minutes = num_intervals * interval_minutes
                            time_delta = timedelta(minutes=total_minutes)
                            end_time = start_datetime + time_delta
                        ...
                        generated_soc_end.append(soc_end)
                        number_of_morning += 1
                        records_generated += 1
                elif rounded_time <= time(23, 30) and rounded_time >= time(21, 30):
                    if number_of_night < number_of_bus:
                        generated_charging_times_start.append(pd.Timestamp.combine(date, rounded_time))
                        generated_soc_start.append(soc_value)
...
def plot_scatter(data):
    ...
    plt.title('SoC(Start) Scatter Plot Over Time (0-24 Hours)')
...
def plot_scatter2(data):
    """
    Plots a scatter graph of SoC over time with vertical lines every 15 minutes.
    """
    plt.figure(figsize=(15, 8))
    unique_dates = data['結束充電時間'].dt.date.unique()
    colors = sns.color_palette("hsv", len(unique_dates))

    for i, date in enumerate(unique_dates):
        daily_data = data[data['結束充電時間'].dt.date == date]
        times = daily_data['結束充電時間'].dt.hour + daily_data['結束充電時間'].dt.minute / 60
        plt.scatter(times, daily_data['SoC(結束)'], color=colors[i], label=f"{date.strftime('%Y-%m-%d')} ({len(daily_data)} points)", alpha=0.6)

    for hour in range(24):
        for quarter in range(0, 60, 15):
            plt.axvline(x=hour + quarter/60, color='grey', linestyle='--', linewidth=0.5)

    plt.title('SoC(End) Scatter Plot Over Time (0-24 Hours)')
    plt.xlabel('Time of Day (Hours)')
    plt.ylabel('Start SoC (%)')
    plt.xlim(0, 24)
    plt.ylim(0, 100)
...
# 將生成的數據按照開始充電時間進行排序
generated_data = generated_data.sort_values(by='開始充電時間')
print(generated_data)

# Plot scatter graph
plot_scatter(generated_data)
plot_scatter2(generated_data)
